...
##  Quality Control
def Quality_Controlx(data, mincells = 3, minfeatures = 200, maxfeatures =2500, mtPct=0.05):
    # Select Genes
    data = data.loc[(data>0).sum(axis=1) >= mincells,:]
    # Select Cells
    #data = data.loc[:,(data>0).sum(axis=0) >= geneNum]
    data = data.loc[:,(data>0).sum(axis=0) >= minfeatures]
    data = data.loc[:,(data>0).sum(axis=0) <= maxfeatures]
    #data = data.loc[:,data.sum(axis=0) <= UMINumUpLimit]
    # Select mitogenes
    mt_row = [r for r in data.index.tolist() if r.startswith("mt") or r.startswith("MT")]
...
def PCA_Permutation(data, name, nperm = 100, nPC = 50):
    ...
    plt.savefig(name + "/PC_SelectionBarplot.png",dpi=600)
    plt.close("all")
    Diff = list((RealEigen - Eigen.values.max()) > 0)
    #Diff = list((RealEigen - np.quantile(Eigen.values, q=0.95)) > 0)
    if False in Diff:
        N = Diff.index(False)
    else:
        N = Dimension
...
def DiskPlot(tSNE,name,Label, legend={"loc":'center right', "bbox_to_anchor":(1.3,0.5),"fontsize":5, "ncol":2}):
    ...
    for n, col in zip(unique_labels, colors):
        ...
        class_mask = [i == n for i in Label]
        tSNE_mask = tSNE.loc[class_mask,:]
        plt.plot(tSNE_mask.iloc[:,0],tSNE_mask.iloc[:,1],'o', markerfacecolor=tuple(col), markeredgecolor='k',markeredgewidth=0.2, markersize=np.sqrt(float(tSNE_mask.iloc[:,4]))+0.1, label = str(n))
    ...
    plt.savefig(name + "_nodes.pdf",dpi=600)
    plt.savefig(name + "_nodes.png",dpi=600)
    plt.close("all")

    
def tSNEPlot(tSNE,name,Label,legend={"loc":'center right', "bbox_to_anchor":(1.4,0.5), "fontsize":5, "ncol":1}):
    unique_labels = list(set(Label))
    unique_labels.sort()
    all_labels = deepcopy(unique_labels)
    colors = [(0.5,0,0,1)]
    ...
    tSNE_mask = tSNE.loc[class_mask,:]
    if tSNE_mask.shape[0] > 0:
        plt.plot(tSNE_mask.iloc[:,0],tSNE_mask.iloc[:,1],'o', markerfacecolor=col, markeredgecolor=col, markeredgewidth=0.1, label = str(n))
    ...
    for n, col in zip(unique_labels, colors):
        ...
        class_mask = [i == n for i in Label]
        tSNE_mask = tSNE.loc[class_mask,:]
        plt.plot(tSNE_mask.iloc[:,0],tSNE_mask.iloc[:,1],'o', markerfacecolor=tuple(col), markeredgecolor='k',markeredgewidth=0.1, label = str(n))
    ...
    plt.savefig(name + "_tSNE.png",dpi=600)
    plt.close("all")
    return 
...
def scDensity2D(data,name,i,Label):
    ...
    ax.set_zlabel('Density')
    ax.set_title("2D scDensity")   
    plt.savefig("Density_" + str(i) + ".pdf",dpi=300)
    plt.close("all")
...
def Density2D(DimensionReduction,name,i):
    ...
    plt.savefig(name + "/" + name + "DecisionPlot-PC" + str(2*i+1) + "_" + "PC" + str(2*i+2) + ".pdf",dpi=300)
...
def ElbowPlot(S6,name):
    ...
    plt.plot(pca.explained_variance_,'o', markerfacecolor='darkcyan',markeredgecolor='k',markeredgewidth=0.1)
    plt.xlabel("Principle Components")
    plt.ylabel("Variance")
    plt.title(name)
    plt.savefig(name+"/ElbowPlot.pdf",dpi=600)
    plt.savefig(name+"/ElbowPlot.png",dpi=600)
    plt.close("all")
...
def CumulativePCAPlot(S6,name):
    ...
    plt.plot(vector,'o', markerfacecolor='darkcyan',markeredgecolor='k',markeredgewidth=0.1)
    plt.xlabel("Principle Components")
    plt.ylabel("Cummulative Explained Variance Ratio")
    plt.title("Variance of each PC")
    plt.savefig(name + "/CummlativePlot.pdf",dpi=600)
    plt.savefig(name + "/CummlativePlot.png",dpi=600)
    plt.close("all")
...
def Scatter_Plot(Data,title,xlab,ylab):
    plt.figure(figsize=(3.3,2.5),dpi=600)
    plt.plot(Data.iloc[:,0],Data.iloc[:,1],'o', markerfacecolor='coral', markeredgecolor='k',markeredgewidth=0.1)
    plt.xlabel(xlab)
    plt.ylabel(ylab)
...
def Heat_Scatter_Plot(Data,gene,name,cluster):
    plt.figure(figsize=(3.3,2.5),dpi=600)
    plt.scatter(Data.iloc[:,0], Data.iloc[:,1], c=Data.iloc[:,4], s=5, cmap='Reds')
    plt.ylabel("tSNE-2")
    plt.xlabel("tSNE-1")
    plt.title(gene)
    #plt.savefig(name + "/" + cluster +"_"+ gene + ".pdf",dpi=600)
    plt.savefig(name + "/" + cluster +"_"+ gene + ".png",dpi=600)
    plt.close("all")

    
def Scatter_Plot_Label(tSNE,name,Label,xlab,ylab,legend={"loc":'center right', "bbox_to_anchor":(1.4,0.5), "fontsize":5, "ncol":1}):
    unique_labels = list(set(Label))
    unique_labels.sort()
    all_labels = deepcopy(unique_labels)
    colors = [(0.5,0,0,1)]
    ...
    plt.subplots_adjust(right = 0.8) 
    #plt.savefig(name + "_tSNE.tiff",dpi=600)
    plt.savefig(name + "_scatter.pdf",dpi=600)
    plt.savefig(name + "_scatter.png",dpi=600)
    plt.close("all")
    return 
...
def Silhouette_Plot(PCA_s,name,Label):
    ...
    plt.savefig("Silhouette.pdf",dpi=300)
    fig.clf()
    plt.close("all")
...
def K_Estimation_Old2(PCAData,Kstart=10):
    K = Kstart
    while True :
        print(K)
        KNNetwork = kneighbors_graph(PCAData, n_neighbors=K, include_self=False)
...
def KNN_Density_Ball_SNN_Old2(PCAData,K):
    starttime = datetime.datetime.now()
    N = PCAData.shape[0]
    d = PCAData.shape[1]
...
def K_Estimation(PCAData,Kstart=10):
    ...
    while True :
        ...
        SNN = [[i,j,len(KNNeighbor[i].intersection(KNNeighbor[j]))/K] for i in range(len(KNNeighbor)) for j in range(i+1,len(KNNeighbor)) if len(KNNeighbor[i].intersection(KNNeighbor[j]))/K  > 0]
        Edges = pd.DataFrame(SNN, columns = ["NodeA","NodeB","Weight"])

        s = int(np.log10(PCAData.shape[0]))
        prune = s/(K+K-s)
        Edges = Edges.loc[Edges.Weight >= prune,:]
        Edges.index = np.arange(Edges.shape[0])    
        edge_list = [(Edges.iloc[i,0],Edges.iloc[i,1]) for i in range(Edges.shape[0])]
...
def KNN_Density_New(PCAData):
    starttime = datetime.datetime.now()
    # Step-1 find nearest neighbors for each
    N = PCAData.shape[0]
    d = PCAData.shape[1]
    Neighbors_nbrs = NearestNeighbors(n_neighbors=N, algorithm='ball_tree', n_jobs = -1).fit(PCAData)
    NearestNeighbors_Dist, NearestNeighbors_Index = Neighbors_nbrs.kneighbors(PCAData, n_neighbors = 2, return_distance = True)
    
    # Step-2 find max distance between nearest neighbors and set it as an distance cutoff
    def Estimate_Dist_Cutoff(PCAData, Neighbors_nbrs, average_neighbors_cutoff=None, initial_cutoff=5, tolerance=0.01):
        if average_neighbors_cutoff is None:
            average_neighbors_cutoff = PCAData.shape[0] * 0.02
        
        def get_average_neighbors(cutoff):
            _, index = Neighbors_nbrs.radius_neighbors(PCAData, cutoff, return_distance=True, sort_results=True)
            neighbor_nums = [len(indices) for indices in index]
            return sum(neighbor_nums) / len(neighbor_nums)
        
        # Check initial cutoff
        average_neighbors = get_average_neighbors(initial_cutoff)
        if abs(average_neighbors - average_neighbors_cutoff) < tolerance:
            return initial_cutoff
        
        # Initialize search parameters
        step = 0.1
        direction = 1 if average_neighbors < average_neighbors_cutoff else -1
        cutoff = initial_cutoff
        
        # Search for optimal cutoff
        while True:
            cutoff += step * direction
            average_neighbors = get_average_neighbors(cutoff)
            if abs(average_neighbors - average_neighbors_cutoff) < tolerance:
                cutoff = np.round(cutoff, 4)
                return cutoff
            elif (average_neighbors - average_neighbors_cutoff) * direction > 0:
                # If we overshot the target, reduce step size and change direction
                step *= 0.5
                direction *= -1
        
    
    Distance_Cutoff = Estimate_Dist_Cutoff(PCAData, Neighbors_nbrs, average_neighbors_cutoff=None, initial_cutoff=5, tolerance=0.01)
    
    # Step-3 find neighbors for each point within Distance_Cutoff
    Neighbors_Dist, Neighbors_Index = Neighbors_nbrs.radius_neighbors(PCAData, Distance_Cutoff, return_distance = True, sort_results = True)
    
    # Step-4 calculate local density for each point
    Local_Density = []
    for i in range(N):
        density = 0
        for d in Neighbors_Dist[i][1:]:
            density = density + np.exp(-1 * (d / Distance_Cutoff)**2)
        Local_Density.append(density)
    Den = Local_Density/sum(Local_Density)*1000
    
    KNNeighbor =[ Neighbors_Index[i][1:].tolist() for i in range(N)]

    SNN = [[i,j,len(list(set(KNNeighbor[i]) & set(KNNeighbor[j]))) / len(list(set(KNNeighbor[i]) | set(KNNeighbor[j])))] for i in range(len(KNNeighbor)) for j in range(i+1,len(KNNeighbor)) if len(list(set(KNNeighbor[i]) | set(KNNeighbor[j]))) > 0 ]
    Edges = pd.DataFrame(SNN, columns = ["NodeA","NodeB","Weight"])
    Edges = Edges.loc[Edges.Weight > 0,:]
    Edges.index = np.arange(Edges.shape[0])
    
    # Step-5 min_spanning_tree to make a graph with least total edge weights that could connect all nodes
    def prune_graph_based_on_connectivity(data): 
        # build a graph model
        G = nx.Graph()
        for row in data.itertuples(index=False):
            G.add_edge(row.NodeA, row.NodeB, weight=row.Weight)
        
        # to generate min_spanning_tree
        min_spanning_tree = nx.minimum_spanning_tree(G)
        # to list edge weight in min_spanning_tree
        edge_weights = [data['weight'] for _, _, data in min_spanning_tree.edges(data=True)]
        # to determine threshold 
        threshold = min(edge_weights)
        return threshold
    
    prune = prune_graph_based_on_connectivity(Edges)
    Edges = Edges.loc[Edges.Weight >= prune,:]
...
def PersistentHomology(Data, tSNE, Edges, Den, name, pd_name ="raw", Stratified = True, iter_plot = False, cluster_plot = True):
    ...
    if Stratified == True:
        ...
        plt.savefig(name +"/Persistent_Diagram_" + str(pd_name) + "_stratified.png", dpi=600)
        plt.close("all")
        ...
        plt.savefig(name +"/Persistent_Diagram_" + str(pd_name) + ".png", dpi=600)
        plt.close("all")
    ...
    if cluster_plot == True:
        ...
        for key in list(Cluster.keys()):
            ...
            if sum(selected) > 1:
                ...
                plt.savefig(name + "/tSNE_runid=" + str(pd_name) + "_clusterid=" + str(key) + ".png", dpi=600)
                plt.close("all")
...
def DownSamplingStability(PCA_Data, PH, Cluster, CellID, tSNE, K, name, iteration = 10):
    ...
    plt.savefig(name + "/Downsampling_stability.png", dpi=600)
    plt.close("all")
...
def Peaks_Cells_Label(PH,Cluster,Cluster_U):
    Tree = dict(zip(list(PH.CurrentCluster),list(PH.UpperCluster)))
    U = list(set(PH.UpperCluster))
    U.remove("None")
    for c in PH.CurrentCluster:
        if c in U:
            barcode_c = PH.loc[PH.CurrentCluster==c,["Barcode"]].values[0][0]
            cluster_c = Cluster[barcode_c]
            barcode_daughters = PH.loc[PH.UpperCluster==c,["Barcode"]].values[:,0]
            other_than_c = np.zeros_like(Cluster[barcode_c])
            for daughter in barcode_daughters:
                other_than_c = other_than_c + Cluster[daughter]
            clean_c_ind = (other_than_c == 0)
            Cluster[barcode_c] = Cluster[barcode_c] * clean_c_ind
            
    Cluster[0] = Cluster_U[0]
    Cluster = pd.DataFrame(Cluster)
    
    Cluster_Peaks = deepcopy(Cluster)
    Cluster_Peaks["Label"] = Cluster_Peaks.sum(axis=1)
    
    PH_Peaks = deepcopy(PH)
    PH_Peaks["ClusterSize"] = [sum(Cluster_Peaks[i]>0) for i in PH.Barcode]
    PH_Peaks = PH_Peaks.loc[PH_Peaks.ClusterSize >=5 ,:]
    Cluster_Peaks = Cluster.loc[:,PH_Peaks.Barcode.values]
    Cluster_Peaks["Label"] = Cluster_Peaks.sum(axis=1)    
    return PH_Peaks,Cluster_Peaks
...
def scRNA_Seq_TDA(file, nGenes=2500, Dim=None, K=None):
    ### Step-1 Specify dataset
    starttime = datetime.datetime.now()
    stem = file.split("_")[0]
    
    name = stem +"_"+ str(nGenes) +"_"+ str(Dim) +"_" + str(K)    
    ...
    S4 = Select_HEGenes(S3,ngenes=nGenes)
    S5 = Scaling_Data(S4)
    S6 = S5.T   
    
    truelabel = pd.read_csv(stem+'_label.txt', header=0, index_col=0, sep="\t")
    ...
    for id in LabelID:
        print(id)
        subRawData = S4.loc[Cluster_All.Label.values == id,:]
        subRawData.T.to_csv(name + "/subCluster_" + str(id) + ".txt", sep="\t")
        subtSNE = tSNE.loc[Cluster_All.Label.values == id,:]
    ...
    print("The estimated cluster number is: " + str(nClust))
    print("The ARI index is: " +  str(ARI)+ "\n")
    print("\nOutput Results Time:")
    endtime = datetime.datetime.now()
    print((endtime-starttime)) 
    return Cluster_All,PH,ARI
...
def scRNA_Seq_TDA_sub(subid):
    ...
    ### Step-2 Data processing
    S4 = pd.read_csv(file, header=0, index_col=0, sep="\s+")
    S5 = Scaling_Data(S4)
    S6 = S5.T
...
def IntraClusterCorrelation(PCA_Data,Label_Column,name,label_order=None):
    """
    Parameters
    ----------
    ...
    """
    if label_order == None:
        unique_labels = list(set(PCA_Data[Label_Column].values))
        unique_labels.sort()
    ...
    colors = [plt.cm.Spectral(each) for each in np.linspace(0, 1, len(unique_labels))]
    pal = {unique_labels[i]:colors[i] for i in range(len(unique_labels))}
    ...
    sns.boxplot(x="Label", y="Corr", palette=pal, data=CORR)
    plt.xlabel("Cluster")
    ...
    plt.savefig(name+"/Intracluster_Consistency_" + Label_Column + ".png", dpi=600)
    plt.close("all")
...
def Heatmap(PCA_Data, Label_Column, legend={"loc":'center left', "bbox_to_anchor":(-0.1,0.5),"fontsize":7, "ncol":1}):
    ...
    plt.savefig("Heatmap_" + Label_Column + ".png", dpi=600)
    plt.close("all")
...
def RepresentativeTranscriptome(S4,LabelColumn):
    data = deepcopy(S4)
    gene = [i for i in data.columns if not i.endswith("Label")]
    Gene = data.loc[:,gene]
    Node = pd.DataFrame(index=gene)
    cluster_label = np.unique(data[LabelColumn].values).tolist()
    if 0 in cluster_label or -1 in cluster_label :
        cluster_label.remove(0)
    for clust in cluster_label:
        select = (data[LabelColumn] == clust)
...
def RepresentativeTranscriptome2(S4,LabelColumn):
    data = deepcopy(S4)
    gene = [i for i in data.columns if not i.endswith("Label")]
    Gene = data.loc[:,gene]
    Node = pd.DataFrame(index=gene)
    ...
    for clust in cluster_label:
        ...
        select_ind = select[select == True].index.to_list()
        cluster = Gene.loc[select_ind, :]
        cluster_count = pd.DataFrame((cluster>0).sum(axis=0), columns=[clust])
        Node = pd.concat([Node, cluster_count], axis=1)
        Node.to_csv(LabelColumn+".csv")
    return(Node)

def RepresentativeTranscriptome3(S4,LabelColumn):
    data = deepcopy(S4)
    gene = [i for i in data.columns if not i.endswith("Label")]
    Gene = data.loc[:,gene]
    Node = pd.DataFrame(index=gene)
...
def ClusterSpecificGeneSet(S4, name, Label, top=10, group=None, nHVG=2000, pct_cutoff = 0.25, log2FC_cutoff = 0.25, pvalue_adjust_cutoff = 0.05 ):
    data = deepcopy(S4)
    data = data.sort_values(by = Label , ascending=True)
    gene = data.columns.to_list()[0:nHVG]
    Gene = data.loc[:,gene]
    ...
    if len(small) > 0:
        for c in small.index.to_list():
            known = Gene.loc[data[Label] == c,:]
            unknown = Gene.loc[data[Label] == 0,:]
            ...
            temp[Label] = c
            LabelA = Label[0:2]+"A"
            temp = temp.loc[temp[Label] == temp[LabelA],:]
            temp = temp.head(10-small[c])
            add_index = temp.index.to_list()
            data.loc[add_index,Label] = c
    
    # selects peak cells for differential genes finding
    data = data.loc[data.loc[:,Label]!=0,:]
    data = data.sort_values(by=Label, ascending=True)
    cluster_label = np.unique(data[Label].values)
    ...
    if group == None:
        for clust in cluster_label:
            ...
            pvalue_adjust = []
            temp = deepcopy(data)
            cluster = temp.loc[temp[Label] == clust,:]
            others  = temp.loc[temp[Label] != clust,:]
            for g in gene:
                c = cluster[g].values
                o = others[g].values
        ...
        if DataFrame.shape[0] != 0:
            ...
            High_Diff_Gene_Cluster = {}
            High_Diff_Gene = []
            for clust in DiffGene:
                High_Diff_Gene_Cluster[clust] = list(DiffGene[clust]["Gene"].values)
                High_Diff_Gene.extend(list(DiffGene[clust]["Gene"].values))
            counts = dict(Counter(High_Diff_Gene))
            redundant = [key for key in counts.keys() if counts[key] > 1]
            No_Redundant = [key for key in counts.keys() if counts[key] == 1]  
            
            High_Diff_Gene = []
            for clust in DiffGene:
                Temp = High_Diff_Gene_Cluster[clust]
                no_redundant = [i for i in Temp if i not in redundant]
                High_Diff_Gene_Cluster[clust] = no_redundant[0:top]
                High_Diff_Gene.extend(no_redundant[0:top])
                
            DataFrame = DataFrame.loc[[True if i in No_Redundant else False for i in DataFrame.Gene.values],:]
            DataFrame.to_csv(name+"/DifferentialGeneSets.csv")
            # average transcriptome for each cluster
            Gene = data.loc[:, gene]
            Node = pd.DataFrame(index=gene)
            for clust in cluster_label:
                select = (data[Label] == clust)
                select_ind = select[select == True].index.to_list()
                cluster = Gene.loc[select_ind, :]
                cluster_mean = pd.DataFrame(cluster.mean(axis=0), columns=[clust])
                Node = pd.concat([Node, cluster_mean], axis=1)
            ...
            # for b0, b1, label in zip(borders[:-1], borders[1:], unique_labels):
            #     g.ax_col_colors.text((b0 + b1) / 2, 1.06, label, color='black', ha='center', va='bottom', transform=g.ax_col_colors.get_xaxis_transform())
            # plt.savefig(name+"/Hierarchical_Dendrogram_clustering-2.pdf", dpi=600)
            # plt.close("all")
            ...
            plt.savefig(name+"/Hierarchical_Dendrogram_sequential.png", dpi=600)
            plt.close("all")
        ...
        pvalue_adjust = []
        temp = deepcopy(data)
        cluster_c = temp.loc[temp[Label] == C,:]
        cluster_o = temp.loc[temp[Label] == O,:]
        for g in gene:
        ...
        result["pvalue"] = pvalue
        result["pvalue_adjust"] = pvalue_adjust
        result = result.sort_values(by="pvalue_adjust" , ascending=True)
        result = result.loc[np.abs(result["log2FC"]) >= log2FC_cutoff, :]
        result = result.loc[(np.abs(result["PCT.1"]) >= pct_cutoff) + (np.abs(result["PCT.2"])  >= pct_cutoff), :]
        #result = result.loc[np.abs(result["PCT.2"])  >= pct_cutoff , :]
        ...
        High_Diff_Gene = result.loc[result.pvalue_adjust<=0.05,"Gene"].values
        temp_select = pd.concat([cluster_c,cluster_o],axis=0)
        Dendrogram = temp_select.loc[:,High_Diff_Gene,]
        Dendrogram = Dendrogram.T
        species = temp_select[Label]
        col_colors = species.map(lut)
        ...
        g = sns.clustermap(Dendrogram, center = 4,cmap="viridis",
                           ...
                           col_colors = col_colors,
                           dendrogram_ratio=(.1, .2),
                           cbar_pos=(.02, .42, .03, .2),figsize=(8,6))
        g.ax_row_dendrogram.remove()
        g.ax_col_dendrogram.remove()
        g.ax_heatmap.set_xticks([])
        Col_Colors = np.array(col_colors)
        borders = np.cumsum([0] + [sum(1 for i in g) for k, g in groupby(Col_Colors)])
        for b0, b1, label in zip(borders[:-1], borders[1:], group):
            g.ax_col_colors.text((b0 + b1) / 2, 1.06, label, color='black', ha='center', va='bottom', transform=g.ax_col_colors.get_xaxis_transform())
        plt.savefig(name+"/Hierarchical_Dendrogram_between_Cluster_" +str(C) + "-" + str(O) + ".pdf", dpi=600)
        plt.savefig(name+"/Hierarchical_Dendrogram_between_Cluster_" +str(C) + "-" + str(O) + ".png", dpi=600)
        plt.close("all")
        
    return DiffGene

def ClusterSpecificGeneSetAllCell(S4, name, top=10, group=None, pct_cutoff = 0.25, log2FC_cutoff = 0.25, pvalue_adjust_cutoff = 0.05 ):
    data = deepcopy(S4)
    data = data.sort_values(by="AllLabel" , ascending=True)
    ...
    if len(small) > 0:
        for c in small.index.to_list():
            ...
            temp = temp.head(10-small[c])
            add_index = temp.index.to_list()
            data.loc[add_index,["AllLabel"]] = c
    
    # selects peak cells for differential genes finding
    data = data.loc[data.loc[:,"AllLabel"]!=0,:]
    data = data.sort_values(by="AllLabel", ascending=True)
    cluster_label = np.unique(data.Label.values)
    ...
    if group == None:
        ...
        DataFrame = DataFrame.loc[[True if i in No_Redundant else False for i in DataFrame.Gene.values],:]
        DataFrame.to_csv(name+"/DifferentialGeneSets_all.csv")
        # average transcriptome for each cluster
        Gene = data.loc[:, gene]
        Node = pd.DataFrame(index=gene)
        for clust in cluster_label:
            ...
            cluster_mean = pd.DataFrame(cluster.mean(axis=0), columns=[clust])
            Node = pd.concat([Node, cluster_mean], axis=1)
        Node.to_csv(name+"/Cluster_Transcriptome_all.csv")
        # plot dendrogram
        ...
        for b0, b1, label in zip(borders[:-1], borders[1:], unique_labels):
            g.ax_col_colors.text((b0 + b1) / 2, 1.06, label, color='black', ha='center', va='bottom', transform=g.ax_col_colors.get_xaxis_transform())
        plt.savefig(name+"/Hierarchical_Dendrogram_sequential_all.pdf", dpi=600)
        plt.savefig(name+"/Hierarchical_Dendrogram_sequential_all.png", dpi=600)
        plt.close("all")
        ...
        result = result.loc[np.abs(result["pvalue_adjust"])  <= pvalue_adjust_cutoff , :]
        result.index = np.arange(result.shape[0])
        result.to_csv(name+"/DifferentialGeneBetweenCluster_" + str(C) +"-"+ str(O) +"_all.csv")
        ...
        for b0, b1, label in zip(borders[:-1], borders[1:], group):
            g.ax_col_colors.text((b0 + b1) / 2, 1.06, label, color='black', ha='center', va='bottom', transform=g.ax_col_colors.get_xaxis_transform())
        plt.savefig(name+"/Hierarchical_Dendrogram_between_Cluster_" +str(C) + "-" + str(O) + "_all.pdf", dpi=600)
        plt.savefig(name+"/Hierarchical_Dendrogram_between_Cluster_" +str(C) + "-" + str(O) + "_all.png", dpi=600)
        plt.close("all")
...
def tSNEforMarker0(tSNE, S4, gene):
    ...
    plt.savefig("tSNE_" + gene + ".png",dpi=600)
    plt.close("all")
...
def tSNEforMarker(tSNE, S4, gene, cluster):
    ...
    plt.savefig("tSNE_Cluster=" + str(cluster) + "_"  + gene + ".png",dpi=600)
    plt.close("all")

def DiskRadius(dataframe):
    data = deepcopy(dataframe)
    data = data - data.mean(axis=0)
    radius = np.round(np.sqrt(np.sum(data**2,axis=1).max()),4)
    return radius

def alphacomplex(data, nodes, name):
    def root(i):
        while i != ID[i]:
            ID[i] = ID[ID[i]]
            i = ID[i]
    ...
    Birthtime = {node:0 for node in nodes.Label.values}
    Deathtime = {node:0 for node in nodes.Label.values}
    DeathList = []
    Clustersize = {node:0 for node in nodes.Label.values}
    Cluster = {}
    Size = nodes["Count"]
    Radius = nodes["Radius"]
    
    nodedist = pd.DataFrame(np.round(squareform(pdist(nodes.iloc[:,0:2], metric='euclidean')),4))
    nodedisttable = nodedist.stack().reset_index().set_axis('From To Distance'.split(), axis=1)
    nodedisttable = nodedisttable.sort_values(by="Distance" , ascending=True)
    nodedisttable = nodedisttable.loc[nodedisttable["From"] < nodedisttable["To"] ,:]
    nodedisttable.index = np.arange(nodedisttable.shape[0])
    nodedisttable.From = nodedisttable.From + 1
    nodedisttable.To = nodedisttable.To + 1

    nodedisttable["SizeFrom"] = Size[nodedisttable.From].values
    nodedisttable["SizeTo"] = Size[nodedisttable.To].values
    ## To define the higher vertice as the "From" vertice, the lower vertice as the "To" vertice 
    Diff = (nodedisttable["SizeFrom"] - nodedisttable["SizeTo"]) < 0
    nodedisttable.loc[Diff,["From","To"]] = nodedisttable.loc[Diff,["To","From"]].values
    nodedisttable.loc[Diff,["SizeFrom","SizeTo"]] = nodedisttable.loc[Diff,["SizeTo","SizeFrom"]].values
    nodedisttable["RadiusFrom"] = Radius[nodedisttable.From].values
    nodedisttable["RadiusTo"] = Radius[nodedisttable.To].values
    nodedisttable["Alpha"] = np.round(np.sqrt( nodedisttable["RadiusFrom"]**4 - 2*(nodedisttable["RadiusFrom"]**2)*(nodedisttable["RadiusTo"]**2) + nodedisttable["RadiusTo"]**4 - 2*(nodedisttable["RadiusFrom"]**2)*(nodedisttable["Distance"]**2) - 2*(nodedisttable["RadiusTo"]**2)*(nodedisttable["RadiusTo"]**2) + nodedisttable["Distance"]**4 )/(2*nodedisttable["Distance"]),4)
    nodedisttable.loc[nodedisttable["Alpha"].isnull(),["Alpha"]] = 0 
    nodedisttable = nodedisttable.sort_values(by="Alpha" , ascending=True)
    nodedisttable.index = np.arange(nodedisttable.shape[0])
    
    data_temp = deepcopy(data)
    ID = nodes.Label.values - 1
    
    
    
    for i in range(nodedisttable.shape[0]):
        if nodedisttable.To[i] not in DeathList:
            Cluster[nodedisttable.To[i]] = deepcopy(ID)
            DeathList.append(nodedisttable.To[i])
            Deathtime[nodedisttable.To[i]] = nodedisttable.Alpha[i]
            union(nodedisttable.From[i]-1,nodedisttable.To[i]-1)

        
        if i % 10000 == 0:
            deunion(nodedisttable.To[i]-1)
            ID = np.array([root(n) for n in ID ])  
            temp = deepcopy(ID)
            data_temp["Label"] = [temp[i-1]+1 for i in data_temp.Label]
            nodes_temp = data_temp.groupby(by = "Label").mean()
            nodes_temp["Cluster"] = nodes_temp.index.to_list()
            nodes_temp["Count"] = data_temp.groupby(by = "Label").count().values[:,0]
            nodes_temp["Radius"] = data_temp.groupby(by = "Label").apply(lambda x:  DiskRadius(x.values[:,0:x.shape[1]-1])) 

            #DiskPlot(nodes_temp, name+"/"+name+"_AlphaComplex_iter=" + str(i), nodes_temp["Cluster"], legend=None)
            tSNEPlot(data_temp, name +"/" + name + "_tSNELabeling_iter=" + str(i),data_temp.Label)
            print(i,np.unique(ID).shape[0])
            union(nodedisttable.From[i]-1,nodedisttable.To[i]-1)
            
        if np.unique(ID).shape[0] == 1:
            break
        
    PH = pd.DataFrame({"Cluster":list(Birthtime.keys()),"BirthTime":list(Birthtime.values()),"DeathTime":list(Deathtime.values())})
    PH = PH.sort_values(by="DeathTime" , ascending=False)
    PH.to_csv(name+'/PersistenceDiagramPeaks_Alpha-Complex.csv',sep=",",index=True,header=True)
    nodedisttable.to_csv(name+"/Edges_Alpha-Complex.csv",sep=",",index=True,header=True)
    font = {'color':  'crimson', 'weight': 'normal'}

    Maximum =  PH.DeathTime.max()
    step = Maximum/100
    plt.figure(figsize=(6,4),dpi=600)
    plt.plot([0, Maximum+0.1], [0, Maximum+0.1], ls="--", color="black")
    plt.plot(PH.BirthTime, PH.DeathTime, "o", markerfacecolor='coral', markeredgecolor='k',markeredgewidth=0.1, markersize=3 )
      
    plt.title("Persistence Diagram of Alpha Complex")
    plt.ylabel("Death")
    plt.xlabel("Birth")
    plt.xticks()
    plt.yticks()
    plt.savefig(name +"/Persistent_Diagram_AlphaComplex.pdf", dpi=600)
    plt.savefig(name +"/Persistent_Diagram_AlphaComplex.png", dpi=600)
    plt.close("all")
