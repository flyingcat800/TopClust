...
### Working Dir
path = "E:/Project/tda_paper/5_Cell_Line/Layer_1/"
...
ax3.plot(tSNE_L1P["tSNE-1"], tSNE_L1P["tSNE-2"], linestyle='None', color="purple", marker='o', markersize=5, markeredgewidth=0.2, markeredgecolor='k', label = "1")
...
ax3.plot(tSNE_L4P["tSNE-1"], tSNE_L4P["tSNE-2"], linestyle='None', color="dodgerblue", marker='o', markersize=5, markeredgewidth=0.2, markeredgecolor='k', label = "4")
ax3.plot(tSNE_L5P["tSNE-1"], tSNE_L5P["tSNE-2"], linestyle='None', color="crimson", marker='o', markersize=5, markeredgewidth=0.2, markeredgecolor='k', label = "5")
...
# peak cell label
# the chosen K = 57
# 1. 数据筛选（保持不变，按原始TrueLabel提取）
tSNE_L1A = tSNE.loc[tSNE.TrueLabel  == "A549"] 
tSNE_L2A = tSNE.loc[tSNE.TrueLabel  == "H1975"] 
tSNE_L3A = tSNE.loc[tSNE.TrueLabel  == "H2228"]
tSNE_L4A = tSNE.loc[tSNE.TrueLabel  == "H838"] 
tSNE_L5A = tSNE.loc[tSNE.TrueLabel  == "HCC827"] 

# 2. 绘图并保存每个类别的 handle（关键：给每个 plot 赋值，取返回列表的第1个元素）
h1 = ax4.plot(tSNE_L1A["tSNE-1"], tSNE_L1A["tSNE-2"], linestyle='None', color="crimson", marker='o', markersize=5, markeredgewidth=0.2, markeredgecolor='k', label = "A549")[0]
h2 = ax4.plot(tSNE_L2A["tSNE-1"], tSNE_L2A["tSNE-2"], linestyle='None', color="coral", marker='o', markersize=5, markeredgewidth=0.2, markeredgecolor='k', label = "H1975")[0]
h3 = ax4.plot(tSNE_L3A["tSNE-1"], tSNE_L3A["tSNE-2"], linestyle='None', color="dodgerblue", marker='o', markersize=5, markeredgewidth=0.2, markeredgecolor='k', label = "H2228")[0]
h4 = ax4.plot(tSNE_L4A["tSNE-1"], tSNE_L4A["tSNE-2"], linestyle='None', color="darkcyan", marker='o', markersize=5, markeredgewidth=0.2, markeredgecolor='k', label = "H838")[0]
h5 = ax4.plot(tSNE_L5A["tSNE-1"], tSNE_L5A["tSNE-2"], linestyle='None', color="purple", marker='o', markersize=5, markeredgewidth=0.2, markeredgecolor='k', label = "HCC827")[0]

# 3. 定义目标顺序：handles 与 labels 一一对应（按需求顺序排列）
# 目标 labels 顺序：HCC827 → H1975 → H838 → H2228 → A549
# 对应 handle：h5（HCC827）→ h2（H1975）→ h4（H838）→ h3（H2228）→ h1（A549）
target_handles = [h5, h2, h4, h3, h1]
target_labels = ["HCC827", "H1975", "H838", "H2228", "A549"]

# 4. 传入重组后的 handles 和 labels，强制按目标顺序显示图例
ax4.legend(handles=target_handles, labels=target_labels, loc=1, fontsize=5)
...
plt.savefig("Figure_2.pdf", dpi=1000)
plt.savefig("Figure_2.tiff", dpi=1000)
